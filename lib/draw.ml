...
let tape_interface_normalize (t : tape_draw_interface) =
  t |> list_of_tape_interface |> tape_interface_of_list
...
let tape_interface_height (t : tape_draw_interface) =
  match tape_interface_normalize t with
...
let rec tikz_of_circuit_meas (t : circuit) (posx : float) (posy : float)
    (debug : bool) (id_zero_width : bool) :
    string * float * float * circuit_draw_interface * circuit_draw_interface =
  if not debug then tikz_of_circuit t posx posy debug id_zero_width
  else
    let diag, h, l, li, ri = tikz_of_circuit t posx posy debug id_zero_width in
  ...
  | Gen (name, ar, coar) -> (
      ...
      | "cocopy" when ar = coar @ coar ->
          ( Printf.sprintf "\\cocopycirc {%s}{%f}{%f}{%f}\n" (fresh_gen ()) posx
            ...
            1.,
            CircuitTens
              (CircuitPin (posx, posy +. !otimes_dist), CircuitPin (posx, posy)),
      ...
      | _ ->
          ...
          ( Printf.sprintf "\\gen {%s}{%f}{%f}{%d}{%d}{%s}{%f}\n" (fresh_gen ())
              posx posy ar_size coar_size name !otimes_dist,
            height,
            2.,
            circuit_interface_rev
              (circuit_interface_init ar_size (fun i ->
                   CircuitPin
                     ( posx,
                       (float_of_int (i - 1) *. !otimes_dist) +. posy +. arshift
                     )))
            |> circuit_interface_normalize,
            circuit_interface_rev
...
let tape_align_interfaces ri1 ri2 =
  ...
  let fc c =
    ...
    | _ ->
        print_endline (show_circuit_draw_interface c);
        failwith "tried to apply circuit map function to non-circuit pin"
  in
  let f t =
    match t with
    | EmptyTape ((_, y1), (_, y2)) -> EmptyTape ((max_x, y1), (max_x, y2))
    | TapeInterface ((_, y1), (_, y2), c) ->
        TapeInterface ((max_x, y1), (max_x, y2), circuit_interface_map fc c)
    | _ -> failwith "tried to apply tape map function to tensor"
  in
...
let rec tikz_of_tape (t : tape) (posx : float) (posy : float) (debug : bool) =
        ...
        if alignable && !align_summands then
          ...
          let diags =
            List.map
              ...
              diags
          in
          let stack_diagrams (i, (da, ha, la, lia, ria))
          ...
          | i, diag, h, l, Some _li, Some ri ->
              ...
              (diag1 ^ diag, max h1 h, l1 +. l +. offset, li1, ri)
          ...
          ( tape_connect_interfaces ri1 li2 ^ diag1 ^ diag2 ^ "\n"
            (*^ Printf.sprintf "\n\\node () at (%f, %f) {%f};\n" (posx +. l1)
                (posy +. (tape_interface_height ri1 /. 2.))
                (tape_interface_height ri1)
             ^ Printf.sprintf "\n\\node () at (%f, %f) {%f};\n"
                (posx +. l1 +. offset)
                (posy +. (tape_interface_height ri1 /. 2.))
                (get_tape_left_interface_height t2) *),
  ...
  | SwapPlus (l1, l2) ->
      ...
      ( Printf.sprintf "\\swaptape {%f} {%f} {%d} {%d} {%f} {%f} {%f} {%f}" posx
          posy len1 len2 !oplus_dist !otimes_dist !tape_padding 2.0,
        h1 +. h2 +. !oplus_dist,
        2.0,
        TapeTens
          ( TapeInterface
  ...
  | Spawn l1 ->
      ...
      let il =
        ...
        |> List.rev
      in
      ( Printf.sprintf "\\spawntape {%f} {%f} {%d} {%f} {%f}" posx posy n
          !tape_padding !otimes_dist,
        h,
        l,
        EmptyInterface,
  ...
  | Split l1 ->
      ...
      let ril2 =
        ...
        |> List.rev
      in
      ( Printf.sprintf "\\splittape {%f} {%f} {%d} {%f} {%f} {%f}" posx posy n
          !tape_padding !otimes_dist !oplus_dist,
        toth,
        l,
        TapeInterface
          ( (posx, posy +. base_left),
  ...
  | Join l1 ->
      ...
      let ril2 =
        ...
        |> List.rev
      in
      ( Printf.sprintf "\\jointape {%f} {%f} {%d} {%f} {%f} {%f}" posx posy n
          !tape_padding !otimes_dist !oplus_dist,
        toth,
        l,
        TapeTens
          ( TapeInterface
              ( (posx, posy +. h +. !oplus_dist),
                ...
                circuit_interface_of_list ril2 ),
            TapeInterface
              ((posx, posy), (posx, posy +. h), circuit_interface_of_list ril1)
          ),
        TapeInterface
