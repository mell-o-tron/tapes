...
and tikz_of_circuit (t:circuit) (posx:float) (posy:float) (debug:bool) = match t with
  ...
  | CCompose (CId _, c2) -> tikz_of_circuit (c2) posx posy debug
  | CCompose (c2, CId _) -> tikz_of_circuit (c2) posx posy debug
  ...
  | CCompose (t1, t2)  -> let diag1, h1, l1, li1, ri1 = tikz_of_circuit_meas t1 posx posy debug in 
                          ...
                          let diag2, h2, l2, li2, ri2 = tikz_of_circuit_meas t2 (posx +. l1) (base_diag1  +. (circuit_interface_height ri1 /. 2. -. get_circuit_height t2 /. 2.)) debug in 
...
let tid_to_normal_form (l : Terms.sort list list) =
  ...
  clean_tape(List.fold_left (fun acc x -> Oplus(acc, x)) TId0 (List.map to_tape l))
